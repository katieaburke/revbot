...
const FIELDS: { key: keyof MeddpiccReq; label: string; letter: string }[] = [
  ...
  { key: 'requireIdentifyPain', label: 'Identify Pain', letter: 'I' },
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">MEDDPICC Config</h2>
          <p className="text-sm text-gray-500 mt-1">Set which MEDDPICC fields are required per stage</p>
        </div>
        <button
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              {FIELDS.map((f) => (
                <th key={f.key} className="text-center px-2 py-3 text-xs font-medium text-gray-500 uppercase" title={f.label}>
                  {f.letter}
                </th>
              ))}
...
function StageModal({ req, onSave, onClose, saving }: {
  req?: MeddpiccReq
